//
// let canvas_component = document.getElementById("canvas");
// canvas_component.style.width = "80%";

// VARIABLE DEFINITIONS
// ============================================================================

const TAU = 2 * Math.PI;
const fancy_pants = 3; // TODO
// MARKER

// numerical parameters
var colony_size = 500; // nr of ants

var r_sa = 20; // ant sensor radius   (minimum for street formation: ~4.5)
var r_s_food = 3;
var ant_fov = (2 / 3) * Math.PI; // (7 / 6) * Math.PI; // 2*Math.PI
// var ant_fov = 2 * Math.PI; // (7 / 6) * Math.PI; // 2*Math.PI
...
var phA_evaporation_rate = 0.5; // min_pheromone_drop_amount ** (1 / 60); // 1 - 1 / world.width**2;
var phB_evaporation_rate = 0.5; // min_pheromone_drop_amount ** (1 / 60); // 1 - 1 / world.width**2;

var probability_for_random_ant_turn = 0.4;
var max_ant_random_turn_angle = Math.PI / 8;

var max_pheromone_strengths = [pheromone_drop_amount, pheromone_drop_amount];

// world parameters
const world_size = [50, 50];
const colony_radius = 3;
const colony_pos = [
  (colony_radius * 3) / 2, // world_size[0] / 2 - colony_radius / 2,
  (colony_radius * 3) / 2, // world_size[1] / 2 - colony_radius / 2,
];
const food_placement_amount = 4000;
const initial_food_amount = 1000;
...
// world & ants
var world, ant_hill;
const ant_eating_radius = 1; // MARKER
  ...
  constructor(world_size) {
    ...
    for (let row_idx = 0; row_idx < this.height; row_idx++) {
      ...
      for (let col_idx = 0; col_idx < this.width; col_idx++) {
        ...
        if (row_idx == world_size[0] / 2 && col_idx == world_size[1] / 2) {
          this.food_sources[row_idx][col_idx] = initial_food_amount;
        }
      }
    }
